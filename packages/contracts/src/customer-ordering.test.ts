...
// Fixtures follow the JSON examples in docs/features/F1-customer-ordering/, with
// real hex UUIDs.
...
describe('placeOrderRequest', () => {
  it('strips fields outside the contract — a client cannot propose a price', () => {
    const parsed = placeOrderRequest.parse({
      items: [{ menuItemId: ITEM, qty: 1, modifierOptionIds: [] }],
      expectedTotalMinor: 9000,
      unitPriceMinor: 1,
    });
    expect('unitPriceMinor' in parsed).toBe(false);
  });
});
