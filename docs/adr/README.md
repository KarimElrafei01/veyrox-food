...
| [0021](ADR-0021-customer-session-tab-restoration.md) | Customer webview restores its opaque session token from tab-scoped storage, then always re-verifies it with the API | Accepted 2026-09-11 |
| [0022](ADR-0022-lan-peer-relay-for-offline-order-visibility.md) | Till↔KDS relay order intents directly over the café LAN during a shared internet outage, using existing device-enrollment trust; cloud API stays the only system of record | Accepted 2026-09-12 · amends 0006 |
